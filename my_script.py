...
sbis_folder = base_dir / "Входящие"
apteki_folder = base_dir / "Аптеки" / "csv" / "correct"


today = datetime.today().strftime('%Y-%m-%d') # полная сегодняшняя дата
result_folder = base_dir / "Результат" / today # папка для сохранения результатов
...
sbis_files = list(sbis_folder.glob("*.csv"))
...
apteki_files = list((base_dir / "Аптеки" / "csv" / "correct").glob("*.csv"))

for file in apteki_files:
